...
const UploadBad = () => {
    ...
    const formatItems = (items) => {
        ...
        return Object.entries(items).map(([item, quantity], index) => (
            <div 
                key={index}
            ...
            >
                ...
                <span className="px-3 py-1 bg-red-400/10 text-red-400 rounded-full text-sm font-medium">
                    {quantity} items
                </span>
            </div>
        ));
    ...
    return (
        <div className="flex flex-col items-center p-8 text-white max-w-4xl mx-auto bg-gradient-to-b from-gray-900 to-gray-800 min-h-screen">
            <h1 className="text-4xl font-bold text-center mb-4 bg-gradient-to-r from-red-400 to-orange-500 bg-clip-text text-transparent">
                Upload & Detect Spoiled Produce
            </h1>
            <p className="text-center mb-8 text-gray-300 max-w-2xl">
                Upload images of spoiled fruits and vegetables to automatically identify and count them using our advanced AI system.
            </p>

            <div 
                {...getRootProps()} 
                className={`w-full max-w-md p-8 rounded-xl border-2 border-dashed transition-all duration-300
                    ${isDragActive ? 'border-red-400 bg-red-400/10 scale-105' : 'border-gray-400'}
                    ${preview ? 'bg-gray-800/60 backdrop-blur-sm' : 'hover:border-red-400 hover:bg-gray-800/30'}`}
            >
                <input {...getInputProps()} />
                <div className="flex flex-col items-center gap-6">
                    {preview ? (
                        ...
                        <div className="text-center space-y-4">
                            <div className="p-4 bg-gradient-to-r from-red-400/10 to-orange-500/10 rounded-full inline-block">
                                <svg 
                            ...
                            <div className="text-gray-300">
                                {isDragActive ? (
                                    <p className="text-red-400 font-medium text-lg">Drop to analyze! 📸</p>
                                ) : (
                ...
                <button
                    ...
                    className={`mt-8 px-8 py-3 rounded-lg font-semibold transition-all duration-300
                        ${loading 
                            ...
                            : 'bg-gradient-to-r from-red-400 to-orange-500 hover:from-red-500 hover:to-orange-600 transform hover:-translate-y-1 hover:shadow-xl active:transform active:scale-95'
                        }`}
                >
                    {loading ? (
            ...
            {(items || finalResponse) && (
                <div className="w-full max-w-md mt-8 space-y-6">
                    ...
                    {finalResponse && (
                        <div className="p-6 rounded-xl bg-gray-800/60 border border-gray-700/50 backdrop-blur-sm shadow-xl">
                            <h3 className="text-xl font-semibold mb-3 text-orange-400">Processing Results:</h3>
